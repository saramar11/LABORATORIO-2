...
# Paso 3: Convertir amplitud a voltaje
def amplitude_to_voltage(data, bit_depth=16):
    max_amplitude = 2 ** (bit_depth - 1)  # 32768 para 16 bits
    voltaje = data / max_amplitude  # Normalizar amplitud para obtener voltaje
    return voltaje


# Generar el vector de tiempo
def generate_time_vector(signal, sample_rate):
    return np.arange(len(signal)) / float(sample_rate)


# Graficar señales separadas en el dominio temporal (voltaje y tiempo)
plt.figure(figsize=(12, 8))

# Fuente 1
voltaje_S1 = amplitude_to_voltage(S[:, 0])
time_S1 = generate_time_vector(S[:, 0], sample_rate_S1)
plt.subplot(3, 1, 1)
plt.plot(time_S1, voltaje_S1)
plt.title('Fuente 1 (Voltaje vs Tiempo)')
plt.xlabel('Tiempo (s)')
plt.ylabel('Voltaje (V)')

# Fuente 2
voltaje_S2 = amplitude_to_voltage(S[:, 1])
time_S2 = generate_time_vector(S[:, 1], sample_rate_S2)
plt.subplot(3, 1, 2)
plt.plot(time_S2, voltaje_S2)
plt.title('Fuente 2 (Voltaje vs Tiempo)')
plt.xlabel('Tiempo (s)')
plt.ylabel('Voltaje (V)')

# Fuente 3
voltaje_S3 = amplitude_to_voltage(S[:, 2])
time_S3 = generate_time_vector(S[:, 2], sample_rate_S3)
plt.subplot(3, 1, 3)
plt.plot(time_S3, voltaje_S3)
plt.title('Fuente 3 (Voltaje vs Tiempo)')
plt.xlabel('Tiempo (s)')
plt.ylabel('Voltaje (V)')
...
original_signal = normalize_signal(mixed3)  # Asegúrate de tener la señal original correcta
...
potencia_recuperada_deci ="{:.4f}".format(potencia_recuperada)
print(f"Potencia Fuente 3: {potencia_recuperada_deci}")

potencia_ruido = np.sum(np.abs(voltajer3 ** 2)) / len(R1)
potencia_ruido_deci ="{:.10f}".format(potencia_ruido)
print(f"Potencia ruido mic  3: {potencia_ruido_deci}")

# Calcular el nuevo SNR
print(" ")
snr_recuperado = 10 * math.log10(potencia_recuperada / potencia_ruido)
